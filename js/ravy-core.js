import { learnResponse, getLearnedResponse } from './memory.js';
...
export function ravyRespond(text, replyCallback) {
  ...
  for(let d of daysQuestions) if(lowerText.includes(d)) { const today = new Date(); response = `Hoy es ${today.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}. 📅`; return replyCallback({ text: response, color: bubbleColor }); }
